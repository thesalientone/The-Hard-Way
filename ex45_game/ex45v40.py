    ...
    def play(self):
        ...
        while current_scene != last_scene:
            ...
            if next_scene_name in ['menu', 'inv', 'save', 'equip',
                ...
                if prev_scene not in [self.scene_map.towns,'open' ,'start', 'menu'] and next_scene_name not in [self.scene_map.towns, 'start']:
...
>"""
    ...
    def act(self, target_list):
        print "%s is performing an action" % self.name
    ...
    def generate_attack_dmg(self, monster):
        ...
        damage = .15 * self.stre * (delta - monster.defe) * (1 +((-1) ** randint(0, 1)) * (random() / 10))
...
>"""
    ...
    def act(self, target_list):
        print "%s is performing an action" % self.__class__.__name__
        action = raw_input(self.action_text)
        if action in ['1', 'attack', 'Attack', 'a', 'A']:
            return self.attack(target_list)
        elif not action:
...
class Battle(Scene):
    ...
    def display_monster_names(self, monsters):
        ...
        for monster in monsters:
            monster_names.append(mosnter.__class__.__name__)
        print display_format.format(monster_names)


    def display_panel(self):

        self.display_monster_names()
        self.display_hp()
        print "-" * 80
